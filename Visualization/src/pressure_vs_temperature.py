...
class CrashSpeed:
    ...
    def calculate_delta_normal_speed(self):
        if self.wall == 'UP' or self.wall == 'DOWN':
            return - self.v_o * np.sen(self.angle) - self.v_o * np.sen(self.angle)
    ...
    for time_frame in time_frames:
        ...
        for index, fila in current_particle_coords.iterrows():
            if fila['crash'] not in NON_WALL_OBJECTS and fila['id'] == fila['ParticleA']:
                current_velocities.append(CrashSpeed(fila['vel'], fila['angle'],
...
def calculate_p_vs_t(speeds):
    ...
    for speed in speeds:
        particles_coords = pd.read_csv(OUTPUT_PATH + 'SimulationData_' + str(N) + '_' + str(L) + "_" + str(speed) + '.csv')
